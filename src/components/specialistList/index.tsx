  ...
  return (
    <div className="inner-wrapper">
      <ul className={styles.list}>
        {topSpecialists?.data?.data?.map(
          ({ image, id, userType, ...specialist }) => {
            ...
            return (
              <li
                key={specialist?.doctorDetails?.id}
                className={styles.listItem}
              >
                <Link
                  href={`${routes.doctor(specialist?.doctorDetails?.userId)}?availability=${specialist?.doctorDetails?.id}`}
                >
                  <DoctorCard
                    size={size}
                    info={{
                    ...
                    image={
                      specialist?.doctorDetails?.avatarUrl ||
